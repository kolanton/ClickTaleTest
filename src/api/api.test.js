...
describe('Worker api class', function () {
  describe('Properties', function () {
    it('should have key limit and card limit properties', function () {
      assert.equal(true, WorkerApi.Props().hasOwnProperty('keyLimit') && WorkerApi.Props().hasOwnProperty('cardLimit'));
    });
    it('should have key limit and card limit as numbers', function () {
      assert.equal(true, !isNaN(WorkerApi.Props().hasOwnProperty('keyLimit')) && !isNaN(WorkerApi.Props().hasOwnProperty('cardLimit')));
    });
  });

  describe('createPagingArray', function () {
    it('return type should be array', function () {
      assert.equal(true, Array.isArray(WorkerApi.createPagingArray(3, 10, 'd')));
    });
    it('if it has results it should has id,active,key properties', function () {
      let res = WorkerApi.createPagingArray(3, 100, 'd')[0];
      assert.equal(true, res.hasOwnProperty('active') && res.hasOwnProperty('key') && res.hasOwnProperty('id'));
    });
  });
  ...
  describe('getWorkerDataByKey', function () {
    ...
    it('if it has results it should has id,active,key properties', function () {
      assert.equal(true, workers.hasOwnProperty('cardsData') && workers.hasOwnProperty('pagingData'));
    });
  });
